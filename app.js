...
function show(screen) {
  [nameScreen, mainScreen, leaderboardScreen].forEach(s =>
...
if (data.todayDate !== today) {
  if (data.todayCount >= DAILY_TARGET) {
    data.daysCompleted++;
  }
...
function updateUI() {
  document.getElementById("todayCount").textContent =
    `${data.todayCount} / ${DAILY_TARGET}`;
  document.getElementById("totalSquats").textContent =
    data.totalSquats;
  document.getElementById("daysCompleted").textContent =
    `${data.daysCompleted} / ${TOTAL_DAYS}`;
}
